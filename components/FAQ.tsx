  ...
  return (
    <section id="faq" className="mt-16 p-6 md:p-12 glass-card rounded-2xl shadow-2xl max-w-8xl mx-auto">
        <div className="text-center">
            ...
            <p className="mt-4 max-w-3xl mx-auto text-lg text-foreground/80">{t('faqSubtitle')}</p>
        </div>
        <div className="mt-12 max-w-4xl mx-auto space-y-4">
            {items.map((num) => (
                <div key={num} className="border-b border-border pb-4">
                    <button 
                        onClick={() => setOpenIndex(openIndex === num ? null : num)}
                        className="w-full flex justify-between items-center text-left text-xl font-semibold text-foreground"
                    >
                    ...
                    <div 
                        className={`mt-4 text-foreground/80 overflow-hidden transition-all duration-300 ${openIndex === num ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}
                    >
                        {t(`faq${num}a` as any)}
                    </div>
                </div>
